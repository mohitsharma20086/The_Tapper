...
int delay_upper = 2300;
int delay_lower = 1800;

int delay_game_2 = 1000;
...
void tapping(int touch_switch, int led_light, int other_1 , int other_2, int other_3) { 
  ...
  else if( delay_lower >= 700){
    delay_lower = delay_lower - 30;
  }
  if( delay_upper >= 1500){
    delay_upper = delay_upper - 50;
  }
  else if( delay_upper >= 1000){
    delay_upper = delay_upper - 30;
  }
  analogWrite( led_light , 0);
  delay(500);
}
...
//      if(digitalRead(sensor_4) == LOW && digitalRead(sensor_1) == LOW && digitalRead(sensor_2) == LOW && digitalRead(sensor_3) == LOW){
//        continue;
//      }      
      score = score - 5;
      if(score < 0 ){
...
//    }
  ...
  if( delay_game_2 > 700){
    delay_game_2 = delay_lower - 50;
  }
  else if( delay_lower >= 400){
    delay_lower = delay_lower - 30;
  }
}
